...
def ParseLefigaro(data):
    ...
    try:
        ...
        rooms = (
            int(re.search("[0-9]+", data.get("roomCountLabel")).group())
            if re.search("[0-9]+", data.get("roomCountLabel"))
            else 0
        )
        ...
        sdata = {
            ...
            "agency_details": {
                ...
                "phone": sellerdetail.get("phoneNumber").replace(" ", "")
                if sellerdetail.get("phoneNumber")
                else "",
            ...
            "visite_virtuelle": visual.get("3dVisualizationUrl")
            or visual.get("plan3dUrl")
            or visual.get("virtualTourUrl"),
            "others": {
                "assets": [opt for opt in data.get("options")]
                if data.get("options")
                else []
            },
