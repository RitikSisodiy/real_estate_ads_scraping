...
class LogicImmoScraper(HttpRequest):
    token = {"token": "", "expiry": 0}
    """
    A class to scrape data from LogicImmo.

    Attributes:
        token (dict): A dictionary that contains the token and expiry time.    
    """

    def __init__(
        self, paremeter, asyncsize=20, timeout=5, proxy=None, maxtry=False
    ) -> None:
        """
        Initializes the class.

        Args:
            paremeter (dict): The parameters to pass to the request.
            asyncsize (int): The number of concurrent requests.
            timeout (int): The timeout for each request.
            proxy (list): A list of proxies to use.
            maxtry (bool): Whether to retry the request if it fails.
        """
        cpath = os.path.dirname(__file__) or "."
        self.logfile = open(f"{cpath}/error.log", "a")
        self.timeout = timeout
        SELOGER_SECURITY_URL = (
            "https://api-logicimmo.svc.groupe-seloger.com/api/security/register"
        )
        headers = {
        ...
        super().__init__(
            True, SELOGER_SECURITY_URL, {}, headers, {}, False, cpath, asyncsize, 5
        )
    ...
    def init_headers(self, sid=0, init=False):
        ...
        Args:
            ...
            init (bool): Whether to initialize the proxy.
    ...
    def getToken(self, sid):
        ...
        if (
            LogicImmoScraper.token
            and LogicImmoScraper.token.get("expiry")
            and LogicImmoScraper.token.get("expiry") - time.time() < 300
        ):
        ...
        SELOGER_SECURITY_URL = (
            "https://api-logicimmo.svc.groupe-seloger.com/api/security"
        )
        time_token = (
            self.session[sid]
            .get(
                f"{SELOGER_SECURITY_URL}/register",
                headers=headers,
                proxies=self.proxy[sid],
                timeout=5,
            )
            .json()
        )
        ...
        res = self.session[sid].get(
            f"{SELOGER_SECURITY_URL}/challenge",
    ...
    def getMaxPrize(self, param, sid=0):
        ...
        try:
            prize = self.fetch(searchurl, method="post", sid=sid, json=param).json()[
                "items"
            ][0]["price"]
            prize = str(prize)
        except:
    ...
    def genFilter(self, adtype):
        """
        Generates the filters and crawls the listings.

        Args:
            adtype (str): The type of ad (sale or rent).

        Returns:
            The number of listings crawled.

        """
        dic = self.paremeter
        if adtype == "sale":
            dic["listingSearchCriterias"]["transactionTypesIds"] = [1, 4, 5, 11]
        else:
            dic["listingSearchCriterias"]["transactionTypesIds"] = [2, 3]
    ...
    def getlatestAd(self, adtype):
        ...
        if adtype == "sale":
            param["listingSearchCriterias"]["transactionTypesIds"] = [1, 4, 5, 11]
        else:
            param["listingSearchCriterias"]["transactionTypesIds"] = [2, 3]
    ...
    def Crawlparam(self, param, allPage=True, first=False, save=True, onlyid=False):
        """
        Crawl the listings for the specified parameters.

        Args:
            param (dict): The parameters to pass to the search API.
            allPage (bool): Whether to crawl all pages.
            first (bool): Whether to only crawl the first page.
            save (bool): Whether to save the listings to the database.
            onlyid (bool): Whether to only crawl the listing IDs.
        ...
        print(param)
        if allPage:
            param["searchParameters"]["offset"] = 0
        response = self.fetch(
            searchurl,
            method="post",
            json=param,
...
def main_scraper(payload, update=False):
    ...
    try:
        ...
        if adtype == "Updated/Latest Ads" or update:
            ...
            return (
                str(ob.updateLatestAd("rental")) + "," + str(ob.updateLatestAd("sale"))
            )
