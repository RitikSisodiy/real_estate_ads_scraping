...
def ParseAvendrealouer(data):
    ...
    # try
        sdata = {
            ...
            "balcony": bool(data.get("balconiesCount")) or any(word in data.get("description").lower() for word in ["balcon","balcony"]),
            "terrace": bool(data.get("terracesCount")) or any(word in data.get("description").lower() for word in ["terrace","terrasse"]),
