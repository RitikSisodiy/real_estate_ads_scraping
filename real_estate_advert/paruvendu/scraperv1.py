...
async def fetch(session,url,proxy=None):
    ...
    try:
        if proxy:
            responce= await session.get(url,headers=headers,proxies={"https":proxy})
        else:
...
async def scrape_pages(session,url,first = True):
    ...
    for ad in ads:
        ...
        if len(tasks) >= 5 or count == len(ads):
    ...
    if first:
        ...
        for i in range(1,totalpage+1):
            ...
            await asyncio.gather(asyncio.ensure_future(scrape_pages(session,nexpageurl,False)))
...
def main_scraper(paylaod):
    ...
    if paylaod["real_state_type"] == "sale":
        ...
        asyncio.run(scrape_rental_ads(paylaod["min_price"], paylaod["max_price"], paylaod["text"]))
async def main():
    # async with aiohttp.ClientSession() as session:
        session = AsyncHTMLSession()
