...
def ParseGreenAcre(data):
  try:
    now = datetime.now()
    assetsDic = {info["type"]:info["value"]  for info in data.get("characteristics")}
    ...
    sdata = {
      ...
      "agency_details": {
        "logo": sellerInfo.get("advertiserDisplayInfo").get("advertiserLogoUrl"),
      ...
      "available": True,
      "status": True,
      "furnished": any(word in (data.get("summary").lower() + " ".join(data.get("options") or "")) for word in ["furnished","meublée","meublé"]),
      "coloc_friendly": False,
      "elevator": any(word in (data.get("summary").lower() + " ".join(data.get("options") or "")) for word in ["elevator","ascenseur"]),
      "pool": any(word in (data.get("summary").lower() + " ".join(data.get("options") or "")) for word in ["piscine","piscina"]),
      "balcony": any(word in (data.get("summary").lower() + " ".join(data.get("options") or "")) for word in ["balcony","balcon",]),
      "floor": any(word in (data.get("summary").lower() + " ".join(data.get("options") or "")) for word in ["estage","floor","étage"]),
      "terrace": any(word in (data.get("summary").lower() + " ".join(data.get("options") or "")) for word in ["terrace","terrasse"]),
      "insee_code": data.get("postalCode") or "",
      "parking": any(word in (data.get("summary").lower() + " ".join(data.get("options") or "")) for word in ["parking","garage"]),
      "images_url": images,
      "is_new": False,
