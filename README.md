...
```
docker run -it --name realscraper -v ./:/app -p 38000:38000 -p 8000:8000 --add-host node-2.kifwat.net:10.8.0.44 --add-host node-3.kifwat.net:10.8.0.45 --add-host node-4.kifwat.net:10.8.0.46 --add-host node-1.kifwat.net:10.8.0.43 realscraper
```
...
Once the container is running, you can access the application at [http://localhost:8000](http://localhost:8000/). You can also access the Celery Flower monitoring tool at [http://localhost:38000](http://localhost:38000/).
...
```
docker run -it --name realscraper -v ./:/app -p 38000:38000 -p 8000:8000 --add-host node-2.kifwat.net:10.8.0.44 --add-host node-3.kifwat.net:10.8.0.45 --add-host node-4.kifwat.net:10.8.0.46 --add-host node-1.kifwat.net:10.8.0.43 -e NAME=John realscraper
```
...
Make sure you navigate to the directory where the docker-compose.yml file is located before executing the command. This command will start the service defined in the Compose file, using the configurations specified in the file.

Please note that the docker-compose.yml file should be properly configured with the necessary service definitions, volume mounts, port mappings, and any additional configurations required for your application.